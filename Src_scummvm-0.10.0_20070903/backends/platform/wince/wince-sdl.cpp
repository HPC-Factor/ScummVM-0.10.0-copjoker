 ...
 *
 * $URL: https://scummvm.svn.sourceforge.net/svnroot/scummvm/scummvm/tags/release-0-10-0/backends/platform/wince/wince-sdl.cpp $
 * $Id: wince-sdl.cpp 27288 2007-06-10 08:20:31Z knakos $
 *
 */
...
static const OSystem::GraphicsMode s_supportedGraphicsModesHigh[] = {
	...
	// cybercop...Try VS2005
#if !deffined(_MSC_VER) || _MSC_VER > 0x700 // EVC breaks template functions, and I'm tired of fixing them :)
...
void OSystem_WINCE3::initBackend()
{
	...
	if (SDL_Init(sdlFlags) == -1) {
		error("Could not initialize SDL_TIMER: %s", SDL_GetError());
	}
	...
	if (SDL_SetTimer(10, &timer_handler_wrapper) == -1) {
		error("Could not set SDL Timer: %s", SDL_GetError());
	}
...
void OSystem_WINCE3::initScreenInfos() {
	...
	_isOzone = 0;
#if SDL_MAJOR_VERSION==1 && SDL_MINOR_VERSION==2 && SDL_PATCHLEVEL < 7				// cybercop
	SDL_Rect **r;
...
void OSystem_WINCE3::update_game_settings() {
	...
	if (!_panelInitialized && !gameid.empty()) {
		...
		if (strncmp(gameid.c_str(), "monkey", 6) == 0) {
			...
			// Cybercop...2007/01/11 WinCE에서 가상키보드가 않보여서 수정
			_panelVisible = true; 
			_toolbarHandler.setActive(NAME_PANEL_KEYBOARD);
			// Cybercop...2007/01/11 WinCE에서 가상키보드가 않보여서 수정
			_toolbarHandler.setVisible(true); 
