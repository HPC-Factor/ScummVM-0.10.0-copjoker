...
#ifdef _MSC_VER
// Visual C++는 strncasecmp를 지원하지 않는다
#define strncasecmp strnicmp
#endif

#ifndef STRNCASECMP
// eMbedded Visual C++는 strnicmp도 지원하지 않는다 (Woolforce)
...
int _korFontWidth = 0;	// 한글의 폰트 너비
int _korFontHeight = 0;	// 한글의 폰트 높이
int _engFontWidth = 0;	// 영문의 폰트 너비
int _engFontHeight = 0;	// 영문의 폰트 높이
...
void loadKoreanStrings()
{
	...
	if(!(fp = fopen(fname, "rt"))) {
		printf("WARNING: 한글 자막 파일을 로드할 수 없습니다!\n");
		return;
	}
	...
	printf("한글 자막 파일이 로드되었습니다 -- 총 %d 라인\n", _numKLines);
...
#ifdef _WIN32_WCE
	strcpy(fontFile, "font/korean.fnt");
//	sprintf(fontFile, "%skorean.fnt", _gameDataPath.c_str());
#else
	...
	if (fp.open(fontFile, Common::File::kFileReadMode, "gp:\\gpmm\\scummvm\\font\\")) {
		...
		fp.close();
		printf("V1 한글 폰트가 로드되었습니다.\n");
	} else {
		printf("V1 한글 폰트를 로드할 수 없습니다!\n");
	}

#ifdef _WIN32_WCE
	strcpy(fontFile, "font/english.fnt");
//	sprintf(fontFile, "%skorean.fnt", _gameDataPath.c_str());
#else
	...
	if (fp.open(fontFile, Common::File::kFileReadMode, "gp:\\gpmm\\scummvm\\font\\")) {
		...
		fp.close();
		printf("V1 영문 폰트가 로드되었습니다.\n");
	} else {
		printf("V1 영문 폰트를 로드할 수 없습니다!\n");
	}
}
