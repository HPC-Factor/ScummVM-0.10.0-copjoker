 ...
 *
 * $URL: https://scummvm.svn.sourceforge.net/svnroot/scummvm/scummvm/tags/release-0-10-0/engines/kyra/staticres.cpp $
 * $Id: staticres.cpp 27024 2007-05-30 21:56:52Z fingolfin $
 *
 */
...
#define LANGUAGE_FLAGS (GF_ENGLISH | GF_FRENCH | GF_GERMAN | GF_SPANISH | GF_ITALIAN | GF_JAPANESE | GF_KOREAN | GF_LNGUNK)
...
uint32 createLanguage(const GameFlags &flags) {
	...
	if (flags.lang == Common::JA_JPN)
		return GF_JAPANESE;
	if (flags.lang == Common::KO_KOR)		// cybercop
		return GF_KOREAN;
	return GF_LNGUNK;
}
...
void KyraEngine::loadMainScreen(int page) {
	...
	else if (_flags.lang == Common::KO_KOR)						// cybercop
		_screen->loadBitmap("MAIN_HAN.CPS", page, page, 0);
	else
